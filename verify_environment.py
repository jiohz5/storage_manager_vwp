...
def check_autostart_directory(path: Path) -> CheckResult:
    target = Path(path).expanduser()
    if target.exists():
        if target.is_dir() and os.access(str(target), os.W_OK | os.X_OK):
            return CheckResult("OK", f"MATE autostart directory writable: {target}")
        return CheckResult("FAIL", f"MATE autostart directory is not writable: {target}")

    ancestor = target.parent
    while not ancestor.exists() and ancestor != ancestor.parent:
        ancestor = ancestor.parent
    if ancestor.is_dir() and os.access(str(ancestor), os.W_OK | os.X_OK):
        return CheckResult("OK", f"MATE autostart directory can be created: {target}")
    return CheckResult("FAIL", f"Cannot create MATE autostart directory: {target}")


def check_environment(data_dir: Path, monitored_root: Path = Path("/user")) -> int:
    checks: List[CheckResult] = []
    python_ok = sys.version_info[:3] >= MIN_PYTHON
    checks.append(
        CheckResult(
            "OK" if python_ok else "FAIL",
            f"Python {sys.version.split()[0]} at {sys.executable} (3.10+ required)",
        )
    )
    checks.append(
        CheckResult(
            "OK",
            f"json standard library: {getattr(json, '__file__', 'built-in')}",
        )
    )
    ...
    checks.append(
        check_autostart_directory(Path.home() / ".config" / "autostart")
    )
