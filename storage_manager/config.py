...
class Settings:
    ...
    capacity_sample_days: int = 30
    rapid_growth_gb: int = 100
    forecast_alert_hours: int = 6
    forecast_emergency_hours: int = 2
    capacity_stale_minutes: int = 45
    popup_backlog_days: int = 7
    data_size_warning_mb: int = 500
...
def ensure_data_dir(data_dir: Path) -> Path:
    try:
        ...
        raise ConfigError(
            f"Storage Manager needs a writable data directory, not the Python home: "
            f"{data_dir}: {exc}. Set STORAGE_MANAGER_DATA_DIR to a writable path."
        ) from exc
    if not data_dir.is_dir() or not os.access(str(data_dir), os.R_OK | os.W_OK | os.X_OK):
        raise ConfigError(
            f"Storage Manager needs a writable data directory: {data_dir}. "
            "Set STORAGE_MANAGER_DATA_DIR to a writable path."
        )
    return data_dir
...
def default_data_dir(
    ...
    # app_dir remains in the signature for compatibility, but source-local state is unsafe.
    from storage_manager.runtime import resolve_data_dir

    resolved = resolve_data_dir(environ=environ, home=home)
    if resolved is None:
        raise ConfigError(
            "No global data directory is configured. Set STORAGE_MANAGER_DATA_DIR "
            "or select a writable location on first GUI launch."
        )
    return resolved
...
def _settings_from_dict(raw: dict) -> Settings:
    ...
    if not 1 <= settings.capacity_sample_days <= 3650:
        raise ConfigError("capacity_sample_days must be between 1 and 3650")
    if settings.rapid_growth_gb < 1:
        raise ConfigError("rapid_growth_gb must be positive")
    if settings.forecast_alert_hours < 1:
        raise ConfigError("forecast_alert_hours must be positive")
    if not 1 <= settings.forecast_emergency_hours <= settings.forecast_alert_hours:
        raise ConfigError(
            "forecast_emergency_hours must be between 1 and forecast_alert_hours"
        )
    if settings.capacity_stale_minutes < 15:
        raise ConfigError("capacity_stale_minutes must be at least 15")
    if not 1 <= settings.popup_backlog_days <= 365:
        raise ConfigError("popup_backlog_days must be between 1 and 365")
    if not 50 <= settings.data_size_warning_mb <= 1_000_000:
        raise ConfigError("data_size_warning_mb must be between 50 and 1000000")
    return settings
...
def load_store(data_dir: Path) -> AccountStore:
    ...
    try:
        ...
        raise ConfigError(
            f"Cannot read {path}: {exc}. Check the selected writable data directory."
        ) from exc
    try:
...
def save_store(data_dir: Path, store: AccountStore) -> None:
    ...
    try:
        ...
        raise ConfigError(
            f"Cannot write {path}: {exc}. Select a writable STORAGE_MANAGER_DATA_DIR."
        ) from exc
    finally:
        try:
            temp_path.unlink()
