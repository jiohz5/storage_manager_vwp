...
def low_priority_prefix(
    ...
    command: List[str] = []
    nice = resolve("nice")
    ionice = resolve("ionice")
    if nice:
        command.extend([nice, "-n", "10"])
    if ionice:
...
def next_capacity_run(now: Optional[datetime] = None) -> datetime:
    current = now or datetime.now()
    for minute in (7, 22, 37, 52):
        candidate = current.replace(minute=minute, second=0, microsecond=0)
        if candidate > current:
            return candidate
    return (current + timedelta(hours=1)).replace(minute=7, second=0, microsecond=0)
