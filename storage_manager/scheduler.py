...
CAPACITY_CRON_MARKER = f"{CRON_MARKER} capacity"
...
def run_nightly_scan(
    ...
    with ProcessLock(lock_file(data_dir)):
        ...
        try:
            ...
            if scan_stop_requested(data_dir, run_id):
                ...
                for index, account in enumerate(candidates):
                    ...
                    if not backend.test_mode and not exact_due:
                        ...
                        record_batch = None
                        if (
                            search_index is not None
                            and account.search_enabled
                            and account.account_id not in search_failed_accounts
                        ):
                            def record_batch(rows, current=account):
                                if current.account_id in search_failed_accounts:
                                    return
                                try:
                                    search_index.upsert_changed_files(
                                        current.account_id,
                                        Path(validated_paths[current.account_id]),
                                        rows,
                                        timestamp=ts,
                                    )
                                except Exception as exc:
                                    search_failed_accounts.add(current.account_id)
                                    print(
                                        f"[WARN] {current.name} incremental search index: {exc}",
                                        file=sys.stderr,
                                    )
            ...
            if search_index is not None and not skip_detail and not stopped:
                update_scan_status(
                    data_dir,
                    run_id,
                    phase="search_index",
                ...
                for account in enabled_accounts:
                    ...
                    try:
                        ...
                        print(
                            f"[WARN] {account.name} full search index: {exc}",
                            file=sys.stderr,
                        )
                        continue
                    if result.cancelled:
...
def cron_line(data_dir: Path, python_bin: str) -> str:
    ...
    return (
        "0 22 * * * "
        + " ".join(_safe_cron_value(value) for value in command)
        + " "
...
def capacity_cron_line(data_dir: Path, python_bin: str) -> str:
    script = Path(__file__).resolve().parent.parent / "capacity_watch.py"
    log_path = data_dir / "capacity_watch.log"
    return (
        "7,22,37,52 * * * * "
        f"{_safe_cron_value(python_bin)} {_safe_cron_value(str(script))} "
        f"--data-dir {_safe_cron_value(str(data_dir))} "
        f">> {_safe_cron_value(str(log_path))} 2>&1 {CAPACITY_CRON_MARKER}"
    )
...
def read_cron_status() -> CronStatus:
    ...
    capacity_line = next(
        (
            row
            for row in result.stdout.splitlines()
            if CAPACITY_CRON_MARKER in row and not row.lstrip().startswith("#")
        ),
        "",
    )
    if line or health_line or capacity_line:
        complete = bool(line and health_line and capacity_line)
        return CronStatus(
            True,
            complete,
            line=line,
            error="" if complete else "managed cron entries are incomplete",
            health_installed=bool(health_line),
            health_line=health_line,
            capacity_installed=bool(capacity_line),
...
def install_cron(data_dir: Path, python_bin: str) -> str:
    ...
    lines.extend([line, capacity_line, health_line])
    payload = "\n".join(lines).rstrip() + "\n"
    subprocess.run(
        ["crontab", "-"],
        input=payload,
...
def run_nightly_cli() -> None:
    ...
    try:
        if args.print_cron:
            ...
            print(capacity_cron_line(data_dir, args.python_bin))
