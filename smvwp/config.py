...
DEFAULT_DETAIL_TASK_TIMEOUT_SECONDS = 15 * 60  # 디렉터리 하나당 측정 예산
...
# 야간 스캔을 계정 몇 개까지 동시에 돌릴지.
#
# ## 왜 직렬(1)이 아닌가 - 대상이 NFS이기 때문이다
#
# 로컬 디스크였다면 병렬은 seek 경합을 부르므로 직렬이 옳다. 그런데 반입
# 대상은 NFS다. 병목이 디스크 탐색이 아니라 **파일 하나당 getattr RPC 왕복**
# 이고, 메타데이터 연산은 읽기·쓰기와 달리 파이프라이닝이 되지 않는다.
# 지연을 감추는 방법은 동시 요청 수를 늘리는 것뿐이다.
#
# `du`는 디렉터리를 한 줄로 걷기 때문에 언제나 요청이 하나만 떠 있다. 실측
# 장비의 RPC 슬롯 상한은 128인데 직렬로는 그중 1개만 쓴다 - 대역폭도 CPU도
# 남는데 왕복 지연을 혼자 다 맞고 있는 상태다.
#
# ## 4인 근거
#
# 실기에서 `bench_parallel.csh`로 잰 값: 동시 2개 1.3배, 동시 4개 2.6배.
...
# ## 이 값은 이제 "동시 계정 수"가 아니라 "동시 **볼륨** 수"다
#
# 뒤이은 실측이 단위를 바꿔 놓았다. 같은 트리를 스레드로든 프로세스로든 더
# 쪼개도 4 위로는 안 빨라졌지만(36.0 / 35.7 / 37.1초), 서로 다른 볼륨 둘은
# 각자 제 속도로 돌아 처리량이 배가 됐다. NetApp 이 볼륨 단위로 처리 능력을
# 가르기 때문이다.
#
# 그래서 `nightly_scan.group_accounts_by_volume`이 계정을 볼륨별로 묶고,
# 이 값은 **묶음 몇 개를 동시에 돌릴지의 상한**이 됐다. 같은 볼륨에 몰린
# 계정은 값을 올려도 자동으로 하나씩 돈다 - 예전 주석이 걱정하던 "곱으로
# 늘어나는 동시 du"는 한 볼륨 안에서는 더 이상 일어나지 않는다.
#
# ## 그런데도 기본값을 1로 두는 이유
#
# 볼륨이 달라도 **파일러는 같을 수 있다**(`ecfiler:/vol/a`와
# `ecfiler:/vol/b`). 볼륨별 한도 위에 파일러 전체의 NIC/CPU 가 또 있는데,
# 그쪽 여유는 이 프로세스에서 관측할 수 없다 (DESIGN.md 1부 2절 "부하에 대한
# 겸손함"). 재 보지 않은 것을 기본값으로 켜지는 않는다.
#
# 올릴 때는 2부터, 그리고 보고서의 "스캔 중 리소스 변화"에서 스캔 시간이
# 실제로 줄었는지 확인하고 올린다.
...
# 사람이 없다는 것이 운영 쪽 판단이라, 요일로 세기를 가를 이유가 없다.
#
# 그래도 값을 따로 두는 이유: 야간 작업이 잡힌 기간에 평일만 낮추는 식으로
# 쓸 여지를 남긴다. 두 값이 같으면 `scan_window.ends_on_weekend` 판정은
# 그대로 돌지만 결과가 갈리지 않는다 - 밤 성격은 계속 기록되므로 주간
# 보고서의 평일/주말 부하 비교는 유효하다. 오히려 **같은 병렬도로 돌 때의
# 비교**라 배경 부하 차이가 더 깨끗하게 드러난다.
DEFAULT_WEEKEND_PARALLEL_ACCOUNTS = DEFAULT_NIGHTLY_PARALLEL_ACCOUNTS
...
class Settings:
    ...
    #
    # **기본은 꺼짐**이다. 정규 경로는 cron(`setup_cron.csh`)이고, 그쪽은 사람이
    # 없어도 돈다. 이 값을 켜면 cron 없이 창만으로 밤 스캔이 돌고 창을 닫으면
    # 같이 멈추는데, 그 대가로 **아무도 창을 안 켜 둔 밤은 통째로 빈다.**
    #
    # 둘 다 켜 두어도 안전하다 - `scan_lock`이 프로세스를 가로질러 하나만 돌게
    # 막는다. 다만 그러면 "창을 닫으면 멈춘다"는 성립하지 않는다.
    gui_auto_nightly_scan: bool = False
