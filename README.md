...
- GUI가 종료되어도 15분마다 `df`만 실행하는 경량 capacity watcher
...
- 메인 창 종료 후에도 독립 실행되고 로그인 때 자동 시작되는 notifier
- 최대 1년간의 일별 사용률 추이
...
`추적` 탭의 상태형 실행 버튼으로 시작한 백그라운드 작업은 GUI를 닫아도 계속됩니다.
...
트레이 팝업으로 표시합니다. 메인 관리 창은 닫아도 됩니다.
...
메인 창의 X를 누르면 종료 안내에 cron과 독립 트레이 알림기의 현재 상태가 표시되고
관리 GUI만 종료됩니다. 등록된 cron과 실행 중인 notifier는 계속 동작합니다.
