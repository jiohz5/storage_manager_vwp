...
**주간 보고서**는 일간 내용을 통째로 포함하고, 거기에 경로별 증감(직전 스캔
대비)을 덧붙입니다. 크기를 재지 못한 경로가 있으면 사유와 함께 나열합니다.

> 화면과 보고서는 **"몇 번째 스캔"**이라고 적습니다. 내부적으로는 세대
> (generation)라는 정수로 관리하지만, 사용자가 알고 싶은 것은 "직전 스캔과
> 비교하면 어떤가"이지 자료구조 이름이 아니기 때문입니다.
